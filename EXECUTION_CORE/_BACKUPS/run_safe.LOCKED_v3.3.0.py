...
# Execution observability (gold standard)
...
class _LiveStageMonitor(threading.Thread):
    ...
    def __init__(
        ...
        preview: TalentIntelPreview,
        get_stage_state: callable,
        interval_sec: int = 15,
        counters_interval_sec: int = 60,
    ) -> None:
        ...
        self._col_candidates = [
            # emails
            ["Primary_Email", "Work_Email", "Home_Email", "Email", "Emails"],
            # phones
            ["Primary_Phone", "Mobile_Phone", "Work_Phone", "Phone", "Phones"],
            # github.io
            ["GitHub_IO_URL", "Github_IO_URL", "Github.io", "GitHub.io", "GitHub_IO", "Github_IO"],
            # cv/resume
            ["Resume_Link", "Resume_URL", "CV_Link", "CV_URL", "Resume", "CV", "Resume_URLs"],
            # oss signals
            ["Open_Source_Impact_Note", "GitHub_Repo_Evidence_Why", "Key_GitHub_AI_Repos", "OSS_Evidence", "OSS_Signals"],
        ]
    ...
    def _recompute_counters(self, out_path: Optional[Path]) -> None:
        ...
        try:
            ...
            # reset by setting deltas relative to current snapshot
    ...
    def run(self) -> None:
        while not self._stop.is_set():
            try:
                ...
                # stage name is intentionally not forced into tracker object; heartbeat uses wrapped snapshot
...
def main(argv: List[str]) -> None:
    ...
    try:
        try:
            ...
            counters = EnrichmentCounters()
            preview = TalentIntelPreview(
                runtime_snapshot_fn=snapshot_wrapped,
                counters_snapshot_fn=(counters.snapshot if counters else (lambda: {})),
                interval_sec=60,
            )
        ...
        # post-run narrative densification
        ...
        # Stop monitor and heartbeat (fail-open)
        try:
            if monitor:
                monitor.stop()
        except Exception:
        ...
        # Preview script (fail-open, post-run)
        ...
        # Completion notifier (always fires, fail-open)
        try:
            if tracker:
