...
#  File: EXECUTION_CORE/citation_velocity_calculator.py
#
#  Purpose:
#    Deterministic citation velocity math for Sample.xlsx schema columns:
#      - Normalized_Citation_Count
#      - Citations_per_Year
#      - Citation_Velocity_3yr
#      - Citation_Velocity_5yr
#      - Citation_Provenance
#    Never guesses citations. Uses available numeric fields only.
#
#  Contract:
#    run(input_csv: str, output_csv: str) -> None
#
#  Version: v2.1.0-citation-velocity
#  Author: Dave Mendoza
# ============================================================
...
def run(input_csv: str, output_csv: str) -> None:
    ...
    # Ensure required columns exist (do not drift schema, just ensure writeability)
    required = [
        "Citation_Count_Raw",
        "Normalized_Citation_Count",
        "Citations_per_Year",
    ...
    for r in rows:
        ...
        norm = raw  # deterministic: normalization is identity unless you add scaling rules later
        ...
        # If we have OpenAlex yearly citation histogram in Graph_Evidence_JSON or Evidence_Tier_Ledger_JSON,
        # we can compute 3yr/5yr velocities precisely. Otherwise, leave blank safely.
...
def _compute_velocities_from_optional_hist(r: Dict[str, str]) -> Tuple[float | None, float | None, float | None, str]:
    ...
    # Compute recent sums and per-year averages
    ...
    # Also compute all-years average if we have a first year
