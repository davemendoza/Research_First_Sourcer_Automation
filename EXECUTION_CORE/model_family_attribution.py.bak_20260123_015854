...
# Canonical display ordering (can be extended safely)
_CANONICAL_ORDER = [
    "GPT-4",
    "GPT-4o",
    "Claude 3",
...
_PATTERNS: List[Tuple[str, re.Pattern]] = [
    ("GPT-4o", _rx(r"\bgpt[- ]?4o\b")),
    ("GPT-4", _rx(r"\bgpt[- ]?4\b")),
    ("Claude 3", _rx(r"\bclaude\b.*\b3\b|\bclaude[- ]?3\b")),
    ("Gemini 1.5", _rx(r"\bgemini\b.*\b1\.?5\b|\bgemini[- ]?1\.?5\b")),
    ("LLaMA 3", _rx(r"\bllama\b.*\b3\b|\bllama[- ]?3\b|\bllama3\b")),
    ("Qwen 2", _rx(r"\bqwen\b.*\b2\b|\bqwen[- ]?2\b|\bqwen2\b")),
    ("DeepSeek-MoE", _rx(r"\bdeepseek\b.*\bmoe\b|\bdeepseek[- ]?moe\b")),
    ("Mixtral", _rx(r"\bmixtral\b")),
    ("Mistral", _rx(r"\bmistral\b")),
    ("DBRX", _rx(r"\bdbrx\b")),
]
...
def attribute_primary_model_families(row: Dict[str, Any]) -> List[str]:
    ...
    # De-dup
    found_set = set(found)
    # Deterministic ordering: canonical first, then any extras alphabetically
...
__all__ = [
    "attribute_primary_model_families",
    "apply_primary_model_families",
]
