...
def _extract_repo_tokens(row: Dict[str, Any]) -> Tuple[Set[str], List[str]]:
    ...
    tokens: Set[str] = set()
    candidate_fields = ['Repo_Topics_Keywords', 'Repo_Topics', 'Repo_Topics_JSON', 'Repo_Languages', 'Repo_Languages_JSON', 'Repo_Descriptions', 'Repo_Description', 'Repo_Readme_Text', 'Repo_README_Text', 'Repo_Text', 'OSS_Evidence', 'OSS_Signals']
...
def _summarize_keywords(tokens: Set[str]) -> str:
    ...
    """
    stop = {'the', 'and', 'or', 'a', 'an', 'of', 'to', 'in', 'for', 'with', 'github', 'com', 'www', 'http', 'https', 'readme', 'license', 'mit', 'apache', 'bsd', 'project', 'repo', 'repository'}
    keep: List[str] = []
...
def _derive_adoption_signal(row: Dict[str, Any], tokens: Set[str]) -> str:
    ...
    """
    fields = ['Downstream_Adoption_Signal', 'Open_Source_Impact_Note', 'GitHub_Repo_Evidence_Why', 'OSS_Evidence', 'OSS_Signals']
...
def _derive_repo_why(row: Dict[str, Any]) -> str:
    ...
    """
    preferred = ['GitHub_Repo_Evidence_Why', 'Open_Source_Impact_Note', 'OSS_Evidence', 'OSS_Signals', 'Repo_Descriptions', 'Repo_Description']
    parts: List[str] = []
    for k in preferred:
...
def decompose_github_signals(row: Dict[str, Any]) -> Dict[str, str]:
    ...
    out = {'Repo_Topics_Keywords': repo_topics_keywords, 'GitHub_Repo_Evidence_Why': repo_why, 'Downstream_Adoption_Signal': adoption, 'Open_Source_Impact_Note': impact_note}
...
def _augment_provenance(row: Dict[str, Any], out: Dict[str, Any]) -> None:
    ...
    if raw:
        try:
            ...
            obj = {}
    obj['GitHub_Signal_Decomposer'] = {'version': _VERSION, 'evidence_basis': 'row_text_fields_only', 'outputs': {k: out.get(k, '') for k in out.keys()}}
    row['Field_Level_Provenance_JSON'] = json.dumps(obj, sort_keys=True)

def apply_github_signal_decomposition(rows: List[Dict[str, Any]], out_map: Dict[str, str] | None=None) -> List[Dict[str, Any]]:
    """
    out_map maps standard output keys to your schema keys.
    Example: {"Repo_Topics_Keywords": "Repo_Topics_Keywords"}
    """
    out_map = out_map or {'Repo_Topics_Keywords': 'Repo_Topics_Keywords', 'GitHub_Repo_Evidence_Why': 'GitHub_Repo_Evidence_Why', 'Downstream_Adoption_Signal': 'Downstream_Adoption_Signal', 'Open_Source_Impact_Note': 'Open_Source_Impact_Note'}
    for r in rows:
        ...
        for k_std, k_row in out_map.items():
            ...
            if v:
                ...
                r[k_row] = r.get(k_row, '')
    return rows
__all__ = ['decompose_github_signals', 'apply_github_signal_decomposition']

def _cli_main():
    _WS_RE = re.compile('\\s+', re.UNICODE)
    _SPLIT_RE = re.compile('[,;/|]+|\\s{2,}', re.UNICODE)
