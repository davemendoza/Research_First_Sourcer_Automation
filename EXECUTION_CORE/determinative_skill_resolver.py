...
def _collect_evidence_text(row: Dict[str, Any]) -> str:
    ...
    preferred_keys = [
        ...
        "GitHub_Profile_Bio",
        "GitHub_Profile_Name",
        "GitHub_Profile_Company",
        "GitHub_Profile_Location",
        "GitHub_Profile_Blog",
        # research planes
        "Scholar_Abstracts",
        "Paper_Abstracts",
    ...
    # also include any other string fields to avoid missing evidence due to schema drift
    for k, v in row.items():
        if k in preferred_keys:
            continue
        if isinstance(v, str) and v.strip():
            # avoid pulling giant JSON blobs twice if already included
...
def _detect_multimodal(text: str) -> Tuple[bool, List[str]]:
    ...
    # gating terms: must show at least one modality pair reference
    gate = [
        "vision",
        "image",
        "audio",
    ...
    # softer but still evidence: explicit mention of 2+ modalities in same text
    modalities = 0
...
def _detect_moe(text: str) -> Tuple[bool, List[str]]:
    ...
    # gate: must include routing or expert load balancing indicator
    gate = [
        "router",
        "routing",
        "load balancing",
...
def _detect_distributed_training(text: str) -> Tuple[bool, List[str]]:
    ...
    # gate: must show training/distributed context
    gate = [
        "distributed",
        "training",
        "pretrain",
...
def resolve_determinative_skill_areas(row: Dict[str, Any]) -> List[str]:
    ...
    # deterministic order per canonical list
    canonical_order = [
        "multimodal fusion (vision + text + audio)",
        "Mixture-of-Experts routing and expert load balancing",
        "distributed training (NCCL/DeepSpeed/FSDP)",
    ...
    # store provenance if requested by schema
...
def apply_determinative_skill_areas(
    rows: List[Dict[str, Any]],
    out_field: str = "Determinative_Skill_Areas",
) -> List[Dict[str, Any]]:
    for r in rows:
...
__all__ = [
    "resolve_determinative_skill_areas",
    "apply_determinative_skill_areas",
]
