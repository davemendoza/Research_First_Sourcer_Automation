...
def classify_production_vs_research(row: Dict[str, Any]) -> str:
    ...
    # - If both have strong signals, Hybrid
    # - If one dominates, pick it
    # - If neither has evidence, return empty (do not fabricate)
    out = ""
    if research_hits == 0 and prod_hits == 0:
        out = ""
    elif research_hits > 0 and prod_hits > 0:
        # require at least 2 total hits to avoid accidental overlap
    ...
    if "Field_Level_Provenance_JSON" in row:
        _augment_provenance(
            ...
            "Production_vs_Research",
            out,
            {"research_hits": research_hits, "prod_hits": prod_hits},
        )
...
__all__ = [
    "classify_production_vs_research",
    "apply_production_vs_research",
]
