...
_BAD_NAME_RE = re.compile(r"^[\W_]+$", re.UNICODE)
...
def resolve_full_name(row: Dict[str, Any]) -> Dict[str, str]:
    ...
    # 5) Handle fallback (non-synthetic)
    handle = (
        _safe_str(row.get("GitHub_Username"))
        or _safe_str(row.get("Github_Username"))
        or _safe_str(row.get("Seed_Query_Or_Handle"))
        or _safe_str(row.get("Handle"))
        or _safe_str(row.get("GitHub_URL"))
    )
...
def _attach_provenance(row: Dict[str, Any], resolved: Dict[str, str]) -> None:
    ...
    obj["Full_Name"] = {
        "version": _VERSION,
        "source": resolved.get("Name_Source"),
        "confidence": resolved.get("Name_Confidence"),
        "value": resolved.get("Full_Name"),
        "evidence_basis": "row_fields_only",
    }
